...
const colorLevels = [
    ...
    { colorClass: "bg-primaryPurple/20", label: "1 a 4 contribuições" },
    { colorClass: "bg-primaryPurple/40", label: "5 a 9 contribuições" },
    { colorClass: "bg-primaryPurple/60", label: "10 a 14 contribuições" },
    { colorClass: "bg-primaryPurple/80", label: "15 a 19 contribuições" },
    { colorClass: "bg-primaryPurple", label: "20 ou mais contribuições" },
];
...
export function ActivityCalendar() {
    ...
    const { calendarGrid, totalCommits } = useMemo(() => {
        ...
        while (current <= endDate) {
            ...
            let count = 0;
            if (intensity > 0.8) count = 25;
            else if (intensity > 0.6) count = 18;
            else if (intensity > 0.4) count = 13;
            else if (intensity > 0.2) count = 8;
            else if (intensity > 0.1) count = 3;
            else count = 0;

            const inYear = current >= yearStart && current <= yearEnd;
            const item = inYear ? { date: isoDate, count, originalDate: new Date(current) } : null;
        ...
        const totalCommitsCalc = realDays.reduce((acc, day) => acc + day.count, 0);
            ...
            <div className="flex flex-col p-2 rounded-md text-white max-w-full select-none">
                <div className="flex justify-between mb-4 items-center flex-wrap gap-2">
                    <p className="text-sm text-neutral-100 font-semibold">
                        {totalCommits} contribuições em {selectedYear}
                    </p>
                    <Select
